...
public final class RatTerm {
   ...
   */
  public int getExpt() {
	  // if coefficient is zero, return 0 (expt is 0)
  ...
  public boolean isNaN() {
	// if coefficient is NaN, RatTerm is NaN
    return coeff.isNaN();
  }
  ...
  public boolean isZero() {
	// RatTerm is 0 if coefficient is 0, 
  ...
  public double eval(double d) {
    // if this is NaN return Double.NaN
    if(this.isNaN()) {
    	return Double.NaN;
    }
    // return the evaluated value at x = d.
  ...
  public RatTerm add(RatTerm arg) {
    // if niether argument is NaN or zero; this.expt != arg.expt throw exceptions 
    ...
    } else { // add agr coefficient to this coefficient 
  ...
  public RatTerm sub(RatTerm arg) {
	...
	} else { // subtract agr coefficient from this coefficient 
  ...
  public RatTerm mul(RatTerm arg) {
    // if ither argument is NaN return NaN
    if (this.isNaN() || arg.isNaN()) {
    	return new RatTerm (RatNum.NaN, 0);
    } else { // return the product of the two terms
  ...
  public RatTerm div(RatTerm arg) {
    // if arg is zero or NaN or this is NaN return NaN
    if (arg.isZero() || this.isNaN() || arg.isNaN()) {
    	return new RatTerm (RatNum.NaN, 0);
    } else { // divide this term by the argument term
  ...
  public RatTerm differentiate() {
    // if it is NaN return NaN
    if(this.isNaN()) {
    	return new RatTerm(RatNum.NaN, 0);
    } else if(this.expt == 0) {
    	return new RatTerm(RatNum.ZERO, 0);   // if this constant number, dy/dx = 0
    } else {
